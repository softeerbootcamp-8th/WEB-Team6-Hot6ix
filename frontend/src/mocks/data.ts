...
const ROOM_3: AuctionRoomDetail = {
  ...
  items: [
    ...
    },
    {
      ...
      endsAt: closedOn('2026-07-18'),
      bidCount: 13,
      topBidderNickname: '렌즈덕후',
      extended: false,
      leaderboard: [
        { rank: 1, nickname: '렌즈덕후', amount: 38000, isMe: false },
 ...
 * 거래별 낙찰 후보 / 최종 순위.
 *
 * 키는 거래 id 다. 금액은 그 물품의 리더보드에서 이어지므로 거래 목록에
 * 찍힌 낙찰가와 1위 후보의 금액이 같다. 유찰 거래는 후보가 없다.
 *
 * 거래 1 은 Figma `WEB-02 · 구매자 · 거래 상세`(12명 · 5명씩 · 내가 7위)를,
 * 거래 2 는 `WEB-03 · 판매자 · 낙찰 후보 관리`를, 유찰 거래(3·8)는
 * `WEB-04 · 낙찰 후보 없음`을 그대로 보여준다.
 */
const CANDIDATES_BY_TRADE: Record<number, DealCandidate[]> = {
  // 핸드메이드 가죽지갑 — 1위가 실패해 2위로 승계됐고, 나는 7위로 대기 중이다.
  1: [
    {
      id: 1,
      rank: 1,
  ...
  // 레트로 필름 카메라 — 입찰이 없어 유찰됐다.
  3: [],
  // 에어포스 1 로우 — 내가 1위로 낙찰받아 거래를 마쳤다.
  4: [
    {
      id: 41,
      rank: 1,
  ...
  // 캐릭터 피규어 세트 — 내가 1위로 낙찰받아 거래를 마쳤다.
  5: [
    {
      id: 51,
      rank: 1,
...
/** 거래 id 로 낙찰 후보를 찾는다. 유찰이면 빈 배열. */
export function mockCandidates(tradeId: number): DealCandidate[] {
  return CANDIDATES_BY_TRADE[tradeId] ?? []
}
