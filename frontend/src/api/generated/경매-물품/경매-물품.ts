...
**여기서 물품이 닫히지는 않는다.** 즉시 마감하지 않고 트리거만큼 남겨두는 것은 구매자에게 얼마나 남았는지 알리기 위해서이며, 앞당긴 뒤에도 Soft Close는 그대로 적용돼 그 구간에 입찰이 들어오면 마감이 다시 밀린다. 소유권·상태·시각 판정과 Stream 기록은 Redis Lua에서 원자적으로 수행하고, DB와 실시간 이벤트는 Consumer가 반영한다. 실제 마감은 새 마감 시각에 스케줄러가 한다.
