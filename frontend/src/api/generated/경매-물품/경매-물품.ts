...
**바디는 선택이다.** 안 보내거나 remainingSeconds를 비우면 경매방의 softCloseTriggerSeconds 만큼 남긴다(이 API의 기존 동작). 절대 시각이 아니라 남은 초로 받는 것은 새 마감 시각을 Redis 시계로 계산하게 해서 서버·브라우저의 시차를 판정에서 빼기 위해서다.

**여기서 물품이 닫히지는 않는다.** 즉시 마감하지 않고 최소 트리거만큼은 남겨두는 것은 구매자에게 얼마나 남았는지 알리기 위해서이며, 앞당긴 뒤에도 Soft Close는 그대로 적용돼 연장 구간에 입찰이 들어오면 마감이 다시 밀린다. 소유권·상태·시각 판정과 Stream 기록은 Redis Lua에서 원자적으로 수행하고, DB와 실시간 이벤트는 Consumer가 반영한다. 실제 마감은 새 마감 시각에 스케줄러가 한다.
