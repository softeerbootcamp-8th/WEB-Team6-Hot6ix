...
export function MobileLiveView({
  ...
  // 라이브 화면이 떠 있는 동안 문서 자체를 잠근다 (섹션 안에서만 스크롤).
  useEffect(() => {
  ...
  return (
    /*
     * 화면 높이에 딱 맞춰 두고 페이지 자체는 스크롤하지 않는다.
     * 물품 목록과 현황 카드가 각자 자기 영역 안에서만 스크롤된다.
     */
    <div className="flex h-[100dvh] flex-col overflow-hidden overscroll-none bg-background">
      {/* 앱바 — 375×64 */}
      ...
      <div className="z-20 h-[88px] shrink-0 border-t bg-card px-4 pt-4">
        <button
          ...
          className="ease-soft h-13 w-full rounded-[14px] bg-primary text-[16px] font-bold text-primary-foreground transition-all duration-150 hover:opacity-90 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-50 disabled:active:scale-100"
        >
