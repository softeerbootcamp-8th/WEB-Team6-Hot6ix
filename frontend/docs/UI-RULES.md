...
- **목록 화면 상단의 주 액션 버튼은 규격이 하나다.** 모바일 `h-11` 전체 폭,
  데스크톱 `md:h-9 md:w-[148px]`, 글자 `text-[13px] font-bold`.
  판매자 정보 · 상품 관리 · 내 경매방이 모두 이 규격을 쓴다. 화면마다 크기를
  다르게 잡으면 같은 흐름을 오갈 때 버튼이 커졌다 작아졌다 한다.
- **상단 제목은 화면 성격에 따라 두 규격만 쓴다.**
  - 목록·허브: `text-[20px] font-bold` + 설명 `text-[13px]`
    (판매자 정보 · 상품 관리 · 내 경매방)
  - 폼·단일 작업: `text-[28px] font-extrabold` + 설명 `text-[14px]`
    (상품 등록 · 프로필 등록/수정 · 회원 탈퇴)

  목록 화면에 폼 규격을 쓰면 같은 계열 화면끼리 제목 크기와 상단 높이가 어긋난다.
