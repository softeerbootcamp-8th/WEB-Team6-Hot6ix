...
- 경매 시작 전과 마감 영속화 완료 후에는 MySQL을 경매 상태의 Source of Truth로 사용한다.
  진행 중(`IN_PROGRESS`)과 마감 영속화 대기(`CLOSING`)에는 Redis Lua가 판정 원본이다.
- Redis-first 입찰·연장·마감은 Lua 상태 변경과 MySQL 영속화 Stream 기록이 함께 성공하면
  승인된 것이다. 그 밖의 성공 상태는 해당 서버 트랜잭션 커밋 전에 확정하지 않는다.
...
- Redis Lua 또는 MySQL 트랜잭션의 해당 상태 판정이 끝나기 전 입찰·낙찰·마감을 성공으로 확정
